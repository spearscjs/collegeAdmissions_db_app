...
CREATE OR REPLACE VIEW meeting_info AS 
	SELECT course_number, section_id, meeting_type, day, start_time, start_time + interval '60 minutes' AS end_time
	FROM (SELECT * FROM recurring_meeting) AS m
	INNER JOIN (SELECT * FROM day_abv_conversion d) AS d
	ON m.meeting_day = d.abbreviation
	ORDER BY course_number,section_id, meeting_type;
	...
	IF EXISTS(
		...
		WHERE e1.course_number = NEW.course_number 
			...
			AND (
					...
					OR
					(
						NEW.start_time + interval '60 minutes' > e1.start_time
						AND
						NEW.start_time + interval '60 minutes' <= e1.end_time
					)
			)	

	)
	THEN RAISE EXCEPTION 'INSERT FAILED: conflicting meeting time';
	END IF;
	return NEW;
END;
$$
...
CREATE TRIGGER check_recurring BEFORE INSERT ON recurring_meeting
    FOR EACH ROW 
...
DROP TRIGGER IF EXISTS check_enrollment_limit ON enrollment;

CREATE OR REPLACE FUNCTION enrollment_limit() RETURNS TRIGGER AS 
$$
BEGIN
	IF EXISTS(
		SELECT COUNT(*) FROM enrollment, class 
			WHERE enrollment.course_number = NEW.course_number AND NEW.section_id = enrollment.section_id
				AND enrollment.course_number = class.course_number AND enrollment.section_id = class.section_id
			GROUP BY(enrollment.course_number, enrollment.section_id, class.enrollment_limit)
			HAVING COUNT(*) >= class.enrollment_limit
	)
	THEN RAISE EXCEPTION 'INSERT FAILED: enrollment limit reached';
	END IF;
	return NEW;
END;
$$
...
CREATE TRIGGER check_enrollment_limit BEFORE INSERT ON enrollment
    FOR EACH ROW 
	EXECUTE PROCEDURE enrollment_limit();




-- FACULTY OVERLAP --------------------------------------------------------------
DROP TRIGGER IF EXISTS check_faculty_overlap ON recurring_meeting;
DROP TRIGGER IF EXISTS check_faculty_overlap ON enrollment;



CREATE OR REPLACE FUNCTION faculty_overlap() RETURNS TRIGGER AS 
$$
DECLARE prof TEXT;
BEGIN
	SELECT faculty_id AS prof into prof FROM class WHERE course_number = NEW.course_number AND section_id = NEW.section_id;
	IF EXISTS(
		SELECT * FROM class c, meeting_info e1 
            WHERE c.course_number = e1.course_number
                AND c.section_id = e1.section_id
                AND c.faculty_id = prof
                AND EXISTS(
                    SELECT * FROM day_abv_conversion d 
                        WHERE NEW.meeting_day = d.abbreviation AND
                            e1.day = d.day
                )
                AND (
                    (
	...
	THEN RAISE EXCEPTION 'INSERT FAILED: meeting overlap';
	END IF;
	return NEW;
END;
$$
...
CREATE TRIGGER check_faculty_overlap BEFORE INSERT ON recurring_meeting
    FOR EACH ROW 
	EXECUTE PROCEDURE faculty_overlap();
...
-- MILESTONE 5 -------------------------------------------------------------------------------------------------------
-- CREATE MATERIALIZED VIEWS (implemented using table)----------------------------------------------------------------
...
-- insert part on ms5
-- CPQG
CREATE OR REPLACE FUNCTION insert_CPQG() RETURNS TRIGGER AS
$$
BEGIN
	IF EXISTS(
	    SELECT *
		FROM CPQG
		WHERE CPQG.course_number = NEW.course_number
		AND CPQG.section_id = NEW.section_id
		AND CPQG.grade = SUBSTRING(NEW.grade, 1, 1)
	) THEN
        UPDATE CPQG
        SET count = count + 1
        WHERE course_number = NEW.course_number
        AND section_id = NEW.section_id
        AND grade = SUBSTRING(NEW.grade, 1, 1);
    ELSE
		INSERT INTO CPQG
        SELECT a.course_number, a.section_id, a.faculty_id, a.season, a.active_year, SUBSTRING(NEW.grade, 1, 1) AS grade, 1 as COUNT
        FROM class a
        WHERE a.course_number = NEW.course_number
...
drop trigger if exists insert_trigger_CPQG on coursework;
CREATE TRIGGER insert_trigger_CPQG BEFORE INSERT ON coursework
    FOR EACH ROW
    EXECUTE PROCEDURE insert_CPQG();
...
BEGIN
	IF EXISTS(
		SELECT *
		FROM CPG
		WHERE CPG.course_number = NEW.course_number
		AND CPG.section_id = NEW.section_id
		AND CPG.grade = SUBSTRING(NEW.grade, 1, 1)
	) THEN
        UPDATE CPG
        SET count = count + 1
        WHERE course_number = NEW.course_number
    ...
    ELSE
		INSERT INTO CPG
        SELECT a.course_number, a.section_id, a.faculty_id, SUBSTRING(NEW.grade, 1, 1) AS grade, 1 as COUNT
        FROM class a
        WHERE a.course_number = NEW.course_number
        AND a.section_id = NEW.section_id;
    END IF;
...
drop trigger if exists insert_trigger_CPG on coursework;
CREATE TRIGGER insert_trigger_CPG BEFORE INSERT ON coursework
    FOR EACH ROW
    EXECUTE PROCEDURE insert_CPG();

-- delete part of ms5
-- CPQG
CREATE OR REPLACE FUNCTION delete_CPQG() RETURNS TRIGGER AS
$$
BEGIN
	IF EXISTS(
	    SELECT *
		FROM CPQG
		WHERE CPQG.course_number = OLD.course_number
		AND CPQG.section_id = OLD.section_id
		AND CPQG.grade = SUBSTRING(OLD.grade, 1, 1)
		AND CPQG.count > 1
	) THEN
UPDATE CPQG
SET count = count - 1
WHERE course_number = OLD.course_number
  ...
  AND grade = SUBSTRING(OLD.grade, 1, 1);
ELSE
DELETE FROM CPQG
WHERE CPQG.course_number = OLD.course_number
  AND CPQG.section_id = OLD.section_id
...
drop trigger if exists delete_trigger_CPQG on coursework;
CREATE TRIGGER delete_trigger_CPQG BEFORE DELETE ON coursework
    FOR EACH ROW
    EXECUTE PROCEDURE delete_CPQG();

-- CPG
CREATE OR REPLACE FUNCTION delete_CPG() RETURNS TRIGGER AS
$$
BEGIN
	IF EXISTS(
	    SELECT *
		FROM CPG
		WHERE CPG.course_number = OLD.course_number
		AND CPG.section_id = OLD.section_id
		AND CPG.grade = SUBSTRING(OLD.grade, 1, 1)
		AND CPG.count > 1
	) THEN
UPDATE CPG
SET count = count - 1
...
drop trigger if exists delete_trigger_CPG on coursework;
CREATE TRIGGER delete_trigger_CPG BEFORE DELETE ON coursework
    FOR EACH ROW
    EXECUTE PROCEDURE delete_CPG();
